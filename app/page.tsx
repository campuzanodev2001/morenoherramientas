...
export default function Home() {
  ...
  const featuredProducts =
    config.sections.featuredProductIds.length > 0
      ? allProducts.filter((p) => config.sections.featuredProductIds.includes(p.id))
    ...
    <>
      ...
      <main className="flex flex-col gap-8 md:gap-12 pt-16">
        ...
        <section className="px-4 md:px-16 flex flex-col gap-4 max-w-[1280px] mx-auto w-full">
          ...
          <Link
            href="/categorias"
            className="w-full border-2 border-primary-container text-primary-container py-4 uppercase font-black tracking-widest text-sm hover:bg-primary-container hover:text-on-primary transition-colors duration-200 flex items-center justify-center gap-2 rounded-none"
          >
        ...
        <section className="px-4 md:px-16 flex flex-col gap-4 max-w-[1280px] mx-auto w-full">
          ...
          <div className="flex flex-col gap-2 md:grid md:grid-cols-2">
            {featuredProducts.map((product) => (
              ...
              >
                <div className="w-20 h-20 flex-shrink-0 relative overflow-hidden rounded-none">
                  <Image
                ...
                <div className="flex flex-col flex-grow min-w-0">
                  <span className="text-on-surface text-xs font-bold uppercase opacity-60">
                    {product.brand}
                  </span>
                  <span className="text-on-surface text-sm font-black uppercase truncate">
                    {product.name}
                  </span>
                ...
                <span className="material-symbols-outlined text-on-surface/40 flex-shrink-0">
                  chevron_right
                </span>
              </Link>
            ))}
        ...
        <section className="bg-accent-red px-4 md:px-16 py-10 flex flex-col gap-4 text-center items-center relative overflow-hidden rounded-none">
          ...
          <p className="text-on-primary font-medium relative z-10 max-w-md text-base">
            Escribinos por WhatsApp y un especialista te ayudará a elegir la herramienta adecuada.
          </p>
        ...
        <div className="max-w-[1280px] mx-auto flex flex-col p-4 md:p-16 gap-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <nav className="flex flex-col gap-3">
              <a className="text-sm font-medium text-white/80 hover:text-white uppercase transition-colors duration-200" href="#">
                Herramientas
              </a>
              <a className="text-sm font-medium text-white/80 hover:text-white uppercase transition-colors duration-200" href="#">
                Marcas
